#Based on the work of Oren Talmor und Gal Avineri
...
def download_images(descriptions: list, images_dir: str, num_processes: int):
    ...
    pool = ThreadPool(processes=num_processes)
    list(tqdm(pool.imap(imap_wrapper, zip(repeat(ImgDownloader.download_image), 
        descriptions, repeat(images_dir))), total=len(descriptions), desc='Downloading Images'))
